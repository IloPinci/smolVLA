...
def find_lerobot_train():
    """
    Locate the lerobot train script.
    Tries the installed entry-point first, then falls back to the source script.
    """
    # Try installed entry-point (lerobot-train or lerobot/scripts/train.py)
    result = subprocess.run(
        ["python", "-c", "import lerobot; import os; "
         "print(os.path.join(os.path.dirname(lerobot.__file__), 'scripts', 'train.py'))"],
    ...
    # Fallback: look for it next to this script or in the lerobot clone
    for p in [
        Path(__file__).parent / "lerobot" / "scripts" / "train.py",
        Path.home() / "lerobot" / "lerobot" / "scripts" / "train.py",
    ]:
        if p.exists():
            return str(p)

    print("[error] Cannot find lerobot/scripts/train.py.")
...
def build_command(train_script: str, args) -> list[str]:
    ...
    cmd = [
        ...
        f"--dataset.local_dir={dataset_dir}",
        f"--output_dir={args.out_dir}",
        f"--steps={args.steps}",
        f"--batch_size={args.batch_size}",
        f"--save_freq={args.save_freq}",
        f"--policy.device={'cuda' if not args.cpu else 'cpu'}",
